...
int push(cmdArgsPtr args)
{
    ...
    if(1 == isusertmp){
        ...
        if (avformat_open_input(&ifmt_ctx, ":0.0+0,0", ifmt, &options) != 0) {
    ...
    if(1 == isusertmp){
        ...
        for (i = 0; i < 1; i++) {
         ...
         int size =
            avpicture_get_size(enc_ctx->pix_fmt, enc_ctx->width,
                               enc_ctx->height);
...
sws_scale(img_convert_ctx,
        ...
        if (enc_pkt.pts == AV_NOPTS_VALUE) {
            ...
            //Parameters
            enc_pkt.pts =
                (double) (videoindex * calc_duration) /
        ...
        ostream = ofmt_ctx->streams[enc_pkt.stream_index];
        /* copy packet */
        enc_pkt.dts =
            av_rescale_q_rnd(enc_pkt.dts, ostream->codec->time_base,
                             ostream->time_base,
                             (enum AVRounding) (AV_ROUND_NEAR_INF |
                                                AV_ROUND_PASS_MINMAX));
        enc_pkt.pts =
            av_rescale_q_rnd(enc_pkt.pts, ostream->codec->time_base,
                             ostream->time_base,
                             (enum AVRounding) (AV_ROUND_NEAR_INF |
                                                AV_ROUND_PASS_MINMAX));
        enc_pkt.duration =
            av_rescale_q(enc_pkt.duration, ostream->codec->time_base,
                         ostream->time_base);
        ...
        printf("========================= %d %d %d \n",enc_pkt.dts,enc_pkt.pts,enc_pkt.duration);
