...
int muxer(cmdArgsPtr args)
{
    AVOutputFormat *ofmt = NULL;
    //Input AVFormatContext and Output AVFormatContext
    AVFormatContext *ifmt_ctx_v = NULL, *ifmt_ctx_a = NULL, *ofmt_ctx =
        NULL;
    AVPacket pkt;
    int ret, i;
    ...
    char in_filename_v[128] = { 0 };
    char in_filename_a[128] = { 0 };
    char *delim;

    if ((delim = strchr(args->infile, '+')) == NULL) {
        printf("no delim char &\n");
        return -1;
    }
    memcpy(in_filename_v, args->infile, delim - args->infile);
    strcpy(in_filename_a, delim + 1);

    const char *out_filename = args->outfile; //Output file URL
    ...
    for (i = 0; i < ifmt_ctx_a->nb_streams; i++) {
        //Create output AVStream according to input AVStream
        if (ifmt_ctx_a->streams[i]->codec->codec_type ==
            AVMEDIA_TYPE_AUDIO) {
